  ...
  >
    ...
    >
      ...
      >
        ...
        right (especially the technical bits), but may be subtly misleading. At
        some point, I might come back and rewrite it.
      </span>

      <p>
        Climate model output is large. The ACCESS-OM2-01 run used here produces
        roughly a terabyte of NetCDF files per simulated year, spread across
        thousands of individual files on
        <a
      ...
      <p>
        ...
        backend, no tiling service, no data pipeline running on a VM somewhere.
        The only infrastructure is the object storage bucket (already there for
        model output) and the static site you're reading this on. The main catch
        is that the S3 bucket needs permissive CORS headers — which, at a
